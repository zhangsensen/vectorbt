...
class CTAEvaluatorV3:
    ...
    def __init__(self, 
                 look_ahead: int = 1,
                 entry_percentile: float = 0.9,  # 收紧分位数，降低信号密度
                 exit_percentile: float = 0.1,
                 sl_stop: float = 0.02,
                 tp_stop: float = 0.03,
                 direction: str = 'both',
    ...
    def _generate_cache_key(self, df: pd.DataFrame, factor_col: str, timeframe: str) -> str:
        """生成缓存键"""
        ...
        data_hash = hashlib.md5(
            f"{df.shape}_{factor_col}_{timeframe}_{self.look_ahead}_{self.entry_percentile}_{self.exit_percentile}".encode()
        ).hexdigest()
        return data_hash
    
    def cta_score(self, df: pd.DataFrame, factor_col: str, timeframe: str = '5m') -> Dict[str, Any]:
        """
        V3修复版CTA打分 - 专注基本逻辑正确性 + 缓存加速
        """
        # 🚀 检查缓存
        cache_key = self._generate_cache_key(df, factor_col, timeframe)
        ...
        try:
            ...
            if missing_cols:
                result = self._empty_result(f"缺失列: {missing_cols}")
                if cache_key:
            ...
            if len(common_index) < 50:  # 大幅降低数据要求
                result = self._empty_result("有效数据不足50个点")
                if cache_key:
                    self._cache[cache_key] = result.copy()
                return result
            ...
            if total_signals < 5:  # 最基本的信号要求
                result = self._empty_result(f"信号数量过少: {total_signals} < 5")
                if cache_key:
                    self._cache[cache_key] = result.copy()
                return result
            ...
            if self.direction == 'both':
                ...
                result = self._empty_result(f"不支持的交易方向: {self.direction}")
                if cache_key:
                    self._cache[cache_key] = result.copy()
                return result
            ...
            return self._run_simple_backtest(price, entries, exits, timeframe, 
                                           factor_processed, total_signals, cache_key)
                
        except Exception as e:
            result = self._empty_result(f"CTA评估失败: {str(e)}")
            if cache_key:
                self._cache[cache_key] = result.copy()
            return result
    ...
    def _generate_percentile_signals(self, factor_processed: pd.Series, 
        ...
        long_entry = (factor_processed > factor_upper).shift(1).fillna(False)
        short_entry = (factor_processed < factor_lower).shift(1).fillna(False)
        
        return long_entry, short_entry
    
    def _run_simple_backtest(self, price: pd.Series, entries: pd.Series, exits: pd.Series, 
                           timeframe: str, factor_processed: pd.Series, total_signals: int, cache_key: str = None) -> Dict[str, Any]:
        """运行简化的VectorBT回测"""
        try:
            ...
            if total_trades < self.min_trades:
                result = self._empty_result(f"交易次数不足{self.min_trades}次 (实际: {total_trades})")
                if cache_key:
                    self._cache[cache_key] = result.copy()
                return result
            ...
            if sharpe <= -1.0:  # 只过滤极差的策略
                result = self._empty_result(f"夏普率过低: {sharpe:.3f} ≤ -1.0")
                if cache_key:
                    self._cache[cache_key] = result.copy()
                return result
            ...
            result = self._empty_result(f"VectorBT回测失败: {str(e)}")
            # 缓存错误结果避免重复计算
            if cache_key:
                self._cache[cache_key] = result.copy()
            return result
    ...
    def batch_evaluate(self, symbols: List[str], factor_data: Dict[str, pd.DataFrame],
        ...
        for symbol in symbols:
            ...
            try:
                ...
                if len(common_index) < 50:
                    continue
                total_tasks += len([f for f in factor_names if f in factors_df.columns])
            except Exception:
                continue
        
        print(f"⚡ 流式处理CTA评估: {total_tasks}个任务")
        ...
        for symbol in symbols:
            ...
            try:
                ...
                # 一次性数据对齐，避免重复操作
                aligned_price = price_df.reindex(common_index)
                aligned_factors = factors_df.reindex(common_index)
                
                for factor_name in factor_names:
                    if factor_name not in aligned_factors.columns:
                        continue
                    ...
                    try:
                        ...
                        temp_df = aligned_price[['close']].join(
                            aligned_factors[[factor_name]], how='inner'
                        )
                        
                        score = self.cta_score(temp_df, factor_name, timeframe)
                        results.append({
                            'symbol': symbol,
                            'factor': factor_name,
                            **score
        ...
        # 过滤有效结果
        score_records = [result for result in results if result is not None]
        
        # 🔍 内存监控：最终内存使用统计
        final_memory = process.memory_info().rss / 1024 / 1024
        total_memory_increase = final_memory - initial_memory
        avg_memory_per_task = total_memory_increase / max(1, total_tasks)
        
        print(f"✅ 流式评估完成: {len(score_records)}/{total_tasks}个任务成功")
        print(f"💾 内存统计:")
        print(f"   - 初始内存: {initial_memory:.1f} MB")
        print(f"   - 最终内存: {final_memory:.1f} MB")
        print(f"   - 总增加: {total_memory_increase:.1f} MB")
        print(f"   - 平均每任务: {avg_memory_per_task:.2f} MB")
